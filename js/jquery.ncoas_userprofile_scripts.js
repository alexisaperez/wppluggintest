		...
		if(!$("#submit").hasClass("btn-disabled")){
			...
			$( this ).find( '.required-field' ).each( function(k, el) {
				...
				if($el.is(':input[type=text]') || $el.is(':input[type=password]')) {
					if( $el.val() == '' ) {
						results.push(
							'&quot;' + $el.attr("data-label") + '&quot; is required.'
						);
						$el.addClass( 'error' );
						if( ! first_error ) {
							first_error = $el;
					...
					if(!(document.getElementById( $el.attr("id")).checked)) {
						results.push(
							'&quot;' + $el.attr("data-label") + '&quot; must be answered.'
						);
						...
						$el.next().removeClass( 'label_error' );
					}
				}
			});
			...
			$( this ).find( '.confirm' ).each( function(k, el) {
				...
				var label = $( el ).attr( 'data-label' );
				var capitalize = curr_id.charAt(0).toUpperCase() + curr_id.slice(1);
				...
				if(!($(first).hasClass("error")) && !($(second).hasClass("error"))){
					if( $(first).val() != $(second).val() ) {
						...
						$(second).empty();
						results.push(capitalize + ' fields do not match, please re-renter the confirmation ' + curr_id);
